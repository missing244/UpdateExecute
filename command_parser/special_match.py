...
def Rotation_Tree(*end_node:BaseMatch.Match_Base):
    ...
    """
    return [
        BaseMatch.Float("Absolute_Rotation:绝对水平朝向值").add_leaves(
            BaseMatch.Float("Absolute_Rotation:绝对垂直朝向值").add_leaves(*end_node),
            Relative_Offset_Float("Relative_Rotation:相对垂直朝向值").add_leaves(*end_node)
        ),
        Relative_Offset_Float("Relative_Rotation:相对水平朝向值").add_leaves(
            BaseMatch.Float("Absolute_Rotation:绝对垂直朝向值").add_leaves(*end_node),
            Relative_Offset_Float("Relative_Rotation:相对垂直朝向值").add_leaves(*end_node)
        )
    ]
...
def BE_Selector_Tree(*end_node:BaseMatch.Match_Base) :
    ...
    *end_node : 添加下一级匹配类\n
    ...
    Selector_Var2 : List[BaseMatch.Match_Base] = [
        ...
        BaseMatch.Enum("Selector_Argument:体积x长度;体积y长度;体积z长度","dx","dy","dz"), # 1
        BaseMatch.Enum("Selector_Argument:距离上限;距离下限;垂直视角上限;垂直视角下限;水平视角上限;水平视角下限","r","rm","rx","rxm","ry","rym"), # 2
        BaseMatch.Enum("Selector_Argument:等级上限;等级下限;选择数量上限","l","lm","c"),       # 3
        BaseMatch.Char("Selector_Argument:实体类型","type"),         # 4
        BaseMatch.Char("Selector_Argument:游戏模式","m"),            # 5
        BaseMatch.Enum("Selector_Argument:实体标签;实体名字;实体族群","tag","name","family"), # 6
        BaseMatch.Char("Selector_Argument:分数条件","scores"),       # 7
        BaseMatch.Char("Selector_Argument:权限条件","haspermission"),# 8
        BaseMatch.Char("Selector_Argument:物品条件","hasitem")       # 9
    ]
    ...
    Selector_Var2[1].add_leaves( BaseMatch.KeyWord("Equal","=").add_leaves( BaseMatch.Int("Value").add_leaves(
        BaseMatch.KeyWord("Next_Selector_Argument:下一个选择器条件",",").add_leaves(*Selector_Var2),
        BaseMatch.KeyWord("End_Selector_Argument:选择器条件结束","]").add_leaves(*end_node)
    )))
    ...
    Selector_Var2[7].add_leaves( BaseMatch.KeyWord("Equal","=").add_leaves( middle_scores_loop(
        BaseMatch.KeyWord("Next_Selector_Argument:下一个选择器条件",",").add_leaves(*Selector_Var2),
        BaseMatch.KeyWord("End_Selector_Argument:选择器条件结束","]").add_leaves(*end_node)
    )))
    Selector_Var2[8].add_leaves( BaseMatch.KeyWord("Equal","=").add_leaves( middle_haspermission_loop(
        BaseMatch.KeyWord("Next_Selector_Argument:下一个选择器条件",",").add_leaves(*Selector_Var2),
        BaseMatch.KeyWord("End_Selector_Argument:选择器条件结束","]").add_leaves(*end_node)
    )))
    Selector_Var2[9].add_leaves( BaseMatch.KeyWord("Equal","=").add_leaves( 
...
def BE_BlockState_Tree(*end_node:BaseMatch.Match_Base) :
    ...
    ...end_node : 添加下一级匹配类\n
    """
    start1 = BaseMatch.KeyWord("Start_BlockState_Argument:方块状态开始","[")
    start2 = BE_BlockState_String("BlockState")
    start2.add_leaves(
        BaseMatch.KeyWord("Equal",":").add_leaves(
            BE_BlockState_String("Value").add_leaves(
                BaseMatch.KeyWord("End_BlockState_Argument:方块状态结束","]").add_leaves(*end_node),
                BaseMatch.KeyWord("Next_BlockState_Argument:下一个方块状态",",").add_leaves(start2)
            ),
