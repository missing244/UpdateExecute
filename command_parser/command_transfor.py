...
BLOCK_STATE_FILE_PATH = os.path.realpath(os.path.join(__file__,"..","block_state.json"))
...
def find_block_state(block_id:str,block_data:int) -> str :
    ...
    return_block_state_str = json.dumps(query_block_state,separators=(',', ':'),ensure_ascii=False)
...
def Selector_Transformer(token_list:List[Dict[Literal["type","token"],Union[str,re.Match]]],index:int) -> Tuple[str,int] :
    ...
    if selector_type == "Player_Name" : return (selector, index + 1)
...
def BlockState_Transformer(block_id:str,token_list:List[Dict[Literal["type","token"],Union[str,re.Match]]],index:int) -> Tuple[str,int] :
    ...
    while 1 :
        ...
        if token_list[index]["type"] == "End_BlockState_Argument" : 
    ...
    block_state_token.pop(0) ; block_state_token.pop(-1)
    return ("[%s]" % "".join(block_state_token), index)
...
def Command_execute_Transformer(token_list:List[Dict[Literal["type","token"],Union[str,re.Match]]]) :
    ...
    while 1 :
        ...
        transfor_list.append("as %s at @s positioned" % transfor_token1)
        for i in range(3) : 
...
Command_to_Transformer = {
    "execute" : Command_execute_Transformer, "fill" : Command_fill_Transformer, "setblock" : Command_setblock_Transformer, 
    "testforblock" : Command_testforblock_Transformer, "clone" : Command_clone_Transformer,
}

def Start_Transformer(token_list:List[Dict[Literal["type","token"],Union[str,re.Match]]]) -> Union[str,Tuple[str]] :
